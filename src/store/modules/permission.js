  ...
  generateRoutes({ commit }, data) {
    return new Promise(resolve => {
      ...
      accessedRouters.map(function(item) {
        ...
        if(item.component != "") router_path_arr.push(item.path) //设置权限路由数组
        if(item.children){
          item.children.map(function(child) {
            if(child.component != "") router_path_arr.push(child.path)  //设置权限路由数组
            ...
            if(child.children){
              ...
              child.children.map(function(children) {
                if(children.component != "") router_path_arr.push(children.path)  //设置权限路由数组
